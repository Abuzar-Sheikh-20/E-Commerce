    ...
    <div className='my-10'>
      <div className="text-center py-8 text-3xl">
        ...
        <p className='w-3/4 m-auto text-xm sm:text-sm md:text-base text-gray-600'>
          Discover the latest trends with our curated collection of high-quality, stylish products, designed to elevate your shopping experience.
        </p>
      </div>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gap-y-6'>
        {
          latestProducts.map((item, index) => (
            <ProductItem key={index} id={item._idd} name={item.name} image={item.image} price={item.price} />       // import ProductItem first
          ))
        }
